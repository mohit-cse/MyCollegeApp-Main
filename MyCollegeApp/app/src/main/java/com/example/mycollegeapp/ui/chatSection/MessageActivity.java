...
public class MessageActivity extends AppCompatActivity {
    ...
    private void sendNotification(String receiver, String username, String message) {
        DatabaseReference tokens = FirebaseDatabase.getInstance().getReference("chat section").child("tokens");
...
//                    Sender sender = new Sender(data, token.getToken());
                    Log.v("Reciever Token",token.getToken());
                    JSONArray tokens=new JSONArray();
                    tokens.put(token.getToken());

                    JSONObject body=new JSONObject();
                    JSONObject data=new JSONObject();

                    try {
                        data.put("user",profileData.get(ProfileSharedPreferences.KEY_REGNO));
                        data.put("icon",R.mipmap.ic_launcher);
                        data.put("role",profileData.get(ProfileSharedPreferences.KEY_ROLE));
                        data.put("body",username + ": " + message);
                        data.put("title","New Message");
                        data.put("sented",userid);

                        body.put("data",data);
                        body.put("registration_ids", tokens);

                        HashMap<String,String> headers=new HashMap<>();
                        headers.put("Authorization","key=AAAAbPd-6CE:APA91bH5BuRSFcKoAqyvpjw9f9xlzwk9Rv4abCe_bSylcgaPEg1KiaCU6kyyfuiyqrbXdd9YsKQ4qNsJ917fEhqBdEBa2JDX8sK2WRtTJ6D7q3aGXvUG4WF2rxjj9uFHJT7Fryg6z8yA");
                        headers.put("Content-Type","application/json");


                        apiService.sendNotification(headers,body.toString())
